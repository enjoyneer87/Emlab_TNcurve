...
[i_d i_q  Lamda_ds Lamda_qs inductance_d inductance_q.0] = deal(zeros(1,size_of(1,1)));  %pre_alloc
...
for i=1:size_of(1,1)-1
    if ele(i,2)==ele(i+1,2)
        for m=1:d
            ...
            inductance_d(d*(i - z) + m + z-1) = (Lamda_ds(d*(i - z) + m + z-1)-Lamda_fd(d*(i - z) + m + z-1))/i_d(d*(i - z) + m + z-1);
            inductance_q(d*(i - z) + m + z-1) = Lamda_qs(d*(i - z) + m + z-1)/i_q(d*(i - z) + m + z-1);
        end
        end_val=end_val+1;
    else
        ...
        inductance_d(d*(end_val-1) + z) = (Lamda_ds(d*(end_val-1) + z)-Lamda_fd(d*(end_val-1) + z))/i_d(d*(end_val-1) + z);
        inductance_q(d*(end_val-1) + z) = (Lamda_qs(d*(end_val-1) + z))/i_q(d*(end_val-1) + z);
        z = z + 1;
    end
end
...
for k=1:length(Input.freq)
    ...
    characteristic_curve = [characteristic_curve; freq torque Imax atan2(Iq,Id)*180/pi+360*(max_power<0) Vll max_power inductance_d inductance_q];
end
Max_Torque = characteristic_curve(1,2);

%% characteristic_curve.csv 파일 생성
headers = {'RPM','Torque','Imax','Phase','V_line','Max_power','L_d','L_q'};
if skew == 0
