...
%% --------------------------입력--------------------------
Input.skew=0;                                                               % Skew 설정 적용 시 1, 미적용시 0
...
Input.i_max = 830;                                                          % 전류 제한
Input.Rs = 0.0100497;                                                     % Rs설정
                                                  % 턴수 입력

% 액티브 파트와 엔드부의 저항비를 구하기 위함
Input.Stack = 140;                                                         % 적층길이 (마진 X)
Input.End_Winding = 70;                                                     % 엔드부 길이
Input.Stack_Margin = 0.97;                                                  % 적층의 마진비율

Input.freq = [1000:500:6000];                                               % 성능곡선 RPM 간격
Input.RPM = [2000];                                                         % 효율맵 RPM 간격          
Input.torque = [300];                                                % 효율맵 토크 간격
...
Input.iter=1;                                                                                   % 손실해석 주기 설정

Input.max_jobs_in_scheduler=2;  %scheduler job 개수
...
Input.JMAG_name_for_lamda = 'HDEV_201019_LdLq.jproj';                    % Lamda 추출 JMAG 파일명
Input.JMAG_name_for_loss = '180314_s4v4r9_LdLq_Loss_7T3P.jproj';                % Coreloss 추출 JMAG 파일명 
